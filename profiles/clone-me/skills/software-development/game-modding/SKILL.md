...
6. **其他引擎？** → GameMaker/RPG Maker 改数据文件，其余用 Cheat Engine 或 Frida MCP（跨平台内存修改，见 `references/mcp-re-tools.md`）
7. **现成 Trainer/Mod？** → NexusMods、社区 Cheat Table
8. **需要静态逆向理解代码？** → GhidraMCP（见 `references/mcp-re-tools.md`）
